...
var ratiosToCalculate = document.querySelectorAll("[data-prop1][data-prop2]");
var settings = [];
ratiosToCalculate.forEach(function (el) {
  settings.push(el.getAttribute("data-prop1").slice(2));
  settings.push(el.getAttribute("data-prop2").slice(2));
});

var setRatioInfo = function setRatioInfo(el) {
  if (el) {
    var prop1 = el.getAttribute("data-prop1");
    var prop2 = el.getAttribute("data-prop2");
    var ratio1 = tinycolor2__WEBPACK_IMPORTED_MODULE_0___default.a.readability(getComputedStyle(document.documentElement).getPropertyValue(prop1), getComputedStyle(document.documentElement).getPropertyValue(prop2));
    el.textContent = ratio1.toFixed(1);
    var wcag = el.closest(".wcag");
...
settings.forEach(function (s) {
  wp.customize(s, function (setting) {
    setting.bind(function (hexValue) {
      var property = "--".concat(s);
      document.documentElement.style.setProperty(property, hexValue);
      var toRecalculate = Array.from(ratiosToCalculate).find(function (el) {
        return el.getAttribute("data-prop1") === property || el.getAttribute("data-prop2") === property;
      });
